class RemediosTwo {
    ...
    obtenerTemplatePostVacio() {
        return `<article class="post">
            ...
            <div class="post-imagenlink">imagen</div>
    ...
    obtenerPostTemplate(id, autor, titulo, descripcion, imagenLink, fecha) {
        ...
        const postHtml = `
            <div class="container my-5 p-4 z-depth-1 rounded">
                ...
                <div class="center-align mt-4">
                    <a class="waves-effect waves-light btn m-3 button-edit" data-id="${id}">Editar</a>
                    <a class="waves-effect waves-light btn red m-3 button-delete" data-id="${id}">Eliminar</a>
                </div>
            </div>`;
    ...
    consultarRemedioParaEditar(id) {
        this.db.collection('remedios').doc(id).get()
            .then(doc => {
                if (doc.exists) {
                    let remedioData = doc.data();
                    $('#tituloEditPost').val(remedioData.titulo);
                    $('#descripcionEditPost').val(remedioData.descripcion);
                    $('#ingredientesEditRemedios').val(remedioData.ingredientes);
                    $('#preparacionEditRemedios').val(remedioData.preparacion);
                    $('#modalEditPost').modal('open');
                    $('#btnSaveEditPost').data('id', id);
                } else {
                    Materialize.toast('El remedio no existe o fue eliminado.', 5000);
                }
            })
    ...
    editarRemedio(id, titulo, descripcion, ingredientes, preparacion, imagenFile) {
        const updateData = {
            titulo: titulo,
            descripcion: descripcion,
            ingredientes: ingredientes,
        ...
        if (imagenFile) {
            const refStorage = firebase.storage().ref(`remedios/${id}/${imagenFile.name}`);
            const task = refStorage.put(imagenFile);
            ...
            task.on('state_changed',
                ...
                () => {
                    task.snapshot.ref.getDownloadURL()
                        .then(url => {
                            updateData.imagenLink = url;
                            this.db.collection('remedios').doc(id).update(updateData)
                                .then(() => {
                                    Materialize.toast('Remedio editado correctamente', 5000);
                                    $('#modalEditPost').modal('close');
                                    this.consultarVerMasRemedio(id);
                                })
                                .catch(error => {
                                    Materialize.toast(`Error al editar el remedio: ${error.message}`, 5000);
                                });
                        });
                }
            );
